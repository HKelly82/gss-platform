...
export default function T4ExercisePage({ params }: { params: Params }) {
  ...
  const block = getAppliedExerciseBlock(parsedTier);
  if (!block) notFound();
  ...
  return (
    <PageBody width="prose">
      <T4StagePage
        stageLabel="Applied exercise"
        currentStep={1}
        moduleTitle={header.moduleTitle}
        tierName={header.tierName || 'Expert tier'}
        body={<Markdown source={block.body} variant="reading" />}
        prev={{ href: `/${pathwaySlug}/${params.module}/T4`, label: 'Tier overview' }}
        next={{ href: `/${pathwaySlug}/${params.module}/T4/critique`, label: 'Critique prompt' }}
      />
    </PageBody>
  );
}
